		...
		if(json.has("in_levels")){
			...
			if(jsonElement.isJsonPrimitive()){
				...
				if(jsonPrimitive.isBoolean()){
					...
					CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > rewards > entries > " + parentRewardId + " > content > in_levels': Value is not a Boolean, defaulting to 'false'!");
					inLevels = false;
				}
			}else{
				CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > rewards > entries > " + parentRewardId + " > content > in_levels': Value is not a JsonPrimitive, please use a boolean, defaulting to 'false'!");
				inLevels = false;
			}
		}else{
			CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > rewards > entries > " + parentRewardId + " > content > in_levels': Not detected, defaulting to 'false'");
			inLevels = false;
		}
			...
			if(jsonElement.isJsonPrimitive()){
				...
				if(jsonPrimitive.isNumber()){
					...
					if(jsonPrimitiveIntValue >= 1){
						...
						CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > rewards > entries > " + parentRewardId + " > content > amount': Value is not >= 1, defaulting to '1'!");
						amount = 1;
					}
				}else{
					CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > rewards > entries > " + parentRewardId + " > content > amount': Value is not an Integer, defaulting to '1'!");
					amount = 1;
				}
			}else{
				CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > rewards > entries > " + parentRewardId + " > content > amount': Value is not a JsonPrimitive, please use a Double, defaulting to '1'!");
				amount = 1;
			}
		}else{
			CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > rewards > entries > " + parentRewardId + " > content > amount': Not detected, defaulting to '1'!");
			amount = 1;
		}
	}
