...
public class VariableButton implements IHoverRenderable, CQGuiEventListener {
	...
	private void internalRender(GuiGraphics matrixStack, int mouseX, int mouseY, float partialTicks, ButtonTexture texture){
		...
		matrixStack.blit(texture.getTexture(), x, y, texU, texV, texP, texP, texWidth, texHeight);// Left Top corner
		matrixStack.blit(texture.getTexture(), right, y, texRight, texV, texP, texP, texWidth, texHeight);// Right Top corner
		matrixStack.blit(texture.getTexture(), right, bottom, texRight, texBottom, texP, texP, texWidth, texHeight);// Right Bottom corner
		matrixStack.blit(texture.getTexture(), x, bottom, texU, texBottom, texP, texP, texWidth, texHeight);// Left Bottom corner
		...
		for (int left = x + texP; left < right; left += strippedButtonTexWidth) {// Fill the Middle
			for (int top = y + texP; top < bottom; top += strippedButtonTexHeight) {
				matrixStack.blit(texture.getTexture(), left, top, texU + texP, texV + texP, Math.min(strippedButtonTexWidth, right - left), Math.min(strippedButtonTexHeight, bottom - top), texWidth, texHeight);
			}
		}
		for (int left = x + texP; left < right; left += strippedButtonTexWidth) {// Top and Bottom Edges
			matrixStack.blit(texture.getTexture(), left, y, texU + texP, texV, Math.min(strippedButtonTexWidth, right - left), texP, texWidth, texHeight);// Top
			matrixStack.blit(texture.getTexture(), left, bottom, texU + texP, texBottom, Math.min(strippedButtonTexWidth, right - left), texP, texWidth, texHeight);// Bottom
		}
		for (int top = y + texP; top < bottom; top += strippedButtonTexHeight) {// Left and Right Edges
			matrixStack.blit(texture.getTexture(), x, top, texU, texV + texP, texP, Math.min(strippedButtonTexHeight, bottom - top), texWidth, texHeight);// Left
			matrixStack.blit(texture.getTexture(), right, top, texRight, texV + texP, texP, Math.min(strippedButtonTexHeight, bottom - top), texWidth, texHeight);// Right
		}
