...
public class BiomeDetectTaskType implements ITaskType{
	private static final ResourceLocation ID = ResourceLocation.fromNamespaceAndPath(Constants.MODID, "biome_detect");
	private static final Component TRANSLATION = Component.translatable(Constants.MODID + ".standardcontent.tasks.biome_detect");
		...
		if(json.has("dimension")){
			...
			if(jsonElement.isJsonPrimitive()){
				...
				if(jsonPrimitive.isString()){
					...
					if(biome == null){
						CustomQuestsLogger.warn("'Quest > " + questId + " > tasks > entries > " + taskId + " > sub_tasks > entries > " + subtaskId + " > biome': Value is not a valid dimension, please use a valid biome id, defaulting to 'minecraft:plains'!");
						biome = Biomes.PLAINS.location();
					}
				}else{
					CustomQuestsLogger.warn("'Quest > " + questId + " > tasks > entries > " + taskId + " > sub_tasks > entries > " + subtaskId + " > biome': Value is not a String, defaulting to 'minecraft:plains'!");
					biome = Biomes.PLAINS.location();
				}
			}else{
				CustomQuestsLogger.warn("'Quest > " + questId + " > tasks > entries > " + taskId + " > sub_tasks > entries > " + subtaskId + " > biome': Value is not a JsonPrimitive, please use a String, defaulting to 'minecraft:plains'!");
				biome = Biomes.PLAINS.location();
			}
		}else{
			CustomQuestsLogger.warn("'Quest > " + questId + " > tasks > entries > " + taskId + " > sub_tasks > entries > " + subtaskId + " > biome': Not detected, defaulting to 'minecraft:plains'!");
			biome = Biomes.PLAINS.location();
		}
	}
