...
public class XpSubmitTaskType implements ITaskType{
	...
	public boolean hasButton(ButtonContext context){
		context.setText(Component.translatable(Constants.MODID  + ".standardcontent.tasks.button.xp_submit"));
	...
	public String getText(LocalPlayer player){
		return amount + " " + Component.translatable(Constants.MODID + ".general.experience").getString() + (inLevels?" "+Component.translatable(Constants.MODID + ".general.levels").getString():" " + Component.translatable(Constants.MODID + ".general.points").getString());
	}
		...
		if(json.has("in_levels")){
			...
			if(jsonElement.isJsonPrimitive()){
				...
				if(jsonPrimitive.isBoolean()){
					...
					CustomQuestsLogger.warn("'Quest > " + questId + " > tasks > entries > " + taskId + " > sub_tasks > entries > " + subtaskId + " > in_levels': Value is not a Boolean, defaulting to 'false'!");
					inLevels = false;
				}
			}else{
				CustomQuestsLogger.warn("'Quest > " + questId + " > tasks > entries > " + taskId + " > sub_tasks > entries > " + subtaskId + " > in_levels': Value is not a JsonPrimitive, please use a boolean, defaulting to 'false'!");
				inLevels = false;
			}
		}else{
			CustomQuestsLogger.warn("'Quest > " + questId + " > tasks > entries > " + taskId + " > sub_tasks > entries > " + subtaskId + " > in_levels': Not detected, defaulting to 'false'");
			inLevels = false;
		}
			...
			if(jsonElement.isJsonPrimitive()){
				...
				if(jsonPrimitive.isNumber()){
					...
					if(jsonPrimitiveIntValue >= 1){
						...
						CustomQuestsLogger.warn("'Quest > " + questId + " > tasks > entries > " + taskId + " > sub_tasks > entries > " + subtaskId + " > amount': Value is not >= 1, defaulting to '1'!");
					}
				}else{
					CustomQuestsLogger.warn("'Quest > " + questId + " > tasks > entries > " + taskId + " > sub_tasks > entries > " + subtaskId + " > amount': Value is not an Integer, defaulting to '1'!");
				}
			}else{
				CustomQuestsLogger.warn("'Quest > " + questId + " > tasks > entries > " + taskId + " > sub_tasks > entries > " + subtaskId + " > amount': Value is not a JsonPrimitive, please use a Double, defaulting to '1'!");
			}
		}else{
			CustomQuestsLogger.warn("'Quest > " + questId + " > tasks > entries > " + taskId + " > sub_tasks > entries > " + subtaskId + " > amount': Not detected, defaulting to '1'!");
		}
