...
public class JEIHelper{
    ...
    public static boolean hasRecipe(ItemStack stack){
        long count = Minecraft.getInstance().level.getRecipeManager().getRecipes().stream().filter(iRecipe -> iRecipe.getResultItem(RegistryAccess.EMPTY).getItem().equals(stack.getItem())).count();
        return count>0;
    }
