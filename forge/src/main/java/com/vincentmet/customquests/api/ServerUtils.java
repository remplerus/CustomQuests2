...
import com.vincentmet.customquests.network.messages.sync.stc.delete.MessageStcSyncDeleteSingleChapter;
import com.vincentmet.customquests.network.messages.sync.stc.delete.MessageStcSyncDeleteSingleParty;
import com.vincentmet.customquests.network.messages.sync.stc.delete.MessageStcSyncDeleteSinglePlayer;
import com.vincentmet.customquests.network.messages.sync.stc.delete.MessageStcSyncDeleteSingleQuest;
import com.vincentmet.customquests.network.messages.sync.stc.delete.MessageStcSyncDeleteSingleReward;
import com.vincentmet.customquests.network.messages.sync.stc.delete.MessageStcSyncDeleteSingleSubreward;
import com.vincentmet.customquests.network.messages.sync.stc.delete.MessageStcSyncDeleteSingleSubtask;
import com.vincentmet.customquests.network.messages.sync.stc.delete.MessageStcSyncDeleteSingleTask;
...
import com.vincentmet.customquests.network.messages.sync.stc.update.MessageStcSyncUpdateSingleReward;
import com.vincentmet.customquests.network.messages.sync.stc.update.MessageStcSyncUpdateSingleSubreward;
import com.vincentmet.customquests.network.messages.sync.stc.update.MessageStcSyncUpdateSingleSubtask;
import com.vincentmet.customquests.network.messages.sync.stc.update.MessageStcSyncUpdateSingleTask;
    ...
    public static class Packets{
        public static class Delete{//These actually delete the chapter/quests/etc., including all the references (i.e. progress)
            ...
            public static void deleteAllChaptersAtAllClients(){
                Constants.currentServerInstance.getPlayerList().getPlayers().forEach(Delete::deleteAllChaptersAtSingleClient);
            }
            ...
            public static void deleteAllQuestsAtAllClients(){
                Constants.currentServerInstance.getPlayerList().getPlayers().forEach(Delete::deleteAllQuestsAtSingleClient);
            }
            ...
            public static void deleteSinglePartyAtAllClients(int partyId){
                Constants.currentServerInstance.getPlayerList().getPlayers().forEach(player -> deleteSinglePartyAtSingleClient(player, partyId));
            }
            ...
            public static void deleteSinglePlayerAtAllClients(UUID playerId){
                Constants.currentServerInstance.getPlayerList().getPlayers().forEach(player -> deleteSinglePlayerAtSingleClient(player, playerId));
            }
            ...
            public static void deleteSingleChapterAtAllClients(int chapterId){
                Constants.currentServerInstance.getPlayerList().getPlayers().forEach(player -> deleteSingleChapterAtSingleClient(player, chapterId));
            }
            ...
            public static void deleteSingleQuestAtAllClients(int questId){
                Constants.currentServerInstance.getPlayerList().getPlayers().forEach(player -> deleteSingleQuestAtSingleClients(player, questId));
            }
            ...
            public static void deleteSingleTaskAtAllClients(int questId, int taskId){
                Constants.currentServerInstance.getPlayerList().getPlayers().forEach(player -> deleteSingleTaskAtSingleClient(player, questId, taskId));
            }
            ...
            public static void deleteSingleSubtaskAtAllClients(int questId, int taskId, int subtaskId){
                Constants.currentServerInstance.getPlayerList().getPlayers().forEach(player -> deleteSingleSubtaskAtSingleClient(player, questId, taskId, subtaskId));
            }
            ...
            public static void deleteSingleRewardAtAllClients(int questId, int rewardId){
                Constants.currentServerInstance.getPlayerList().getPlayers().forEach(player -> deleteSingleRewardAtSingleClient(player, questId, rewardId));
            }
            ...
            public static void deleteSingleSubrewardAtAllClients(int questId, int rewardId, int subrewardId){
                Constants.currentServerInstance.getPlayerList().getPlayers().forEach(player -> deleteSingleSubrewardAtSingleClient(player, questId, rewardId, subrewardId));
            }
        ...
        public static class SyncToClient{//These temporarily remove the quest, and then rebuild the quest from memory with the updated values form the server
            public static class Progress{
                ...
                public static class Parties{
                    public static void syncAllParties(){
                        Constants.currentServerInstance.getPlayerList().getPlayers().forEach(Parties::syncAllPartiesToPlayer);
                    }
                    ...
                    public static void syncParty(int partyId){
                        Constants.currentServerInstance.getPlayerList().getPlayers().forEach(player -> {
                ...
                public static class Players{
                    public static void syncAllPlayerProgress(){
                        Constants.currentServerInstance.getPlayerList().getPlayers().forEach(Players::syncAllPlayerProgressToPlayer);
                    }
                    ...
                    public static void syncPlayer(UUID uuid){
                        Constants.currentServerInstance.getPlayerList().getPlayers().forEach(player -> syncPlayerToPlayer(player, uuid));
                    }
            ...
            public static class Data{
                public static void syncAllChaptersAndQuests(){
                    Constants.currentServerInstance.getPlayerList().getPlayers().forEach(Data::syncAllChaptersAndQuestsToPlayer);
                }
                ...
                public static class Quests{
                    public static void syncAllQuests(){
                        Constants.currentServerInstance.getPlayerList().getPlayers().forEach(Quests::syncAllQuestsToPlayer);
                    }
                    ...
                    public static void syncQuest(int questId){//todo maybe add tasks in for loop instead of full quest sync, otherwise datacap crash will happen again
                        Constants.currentServerInstance.getPlayerList().getPlayers().forEach(player -> syncQuestToPlayer(player, questId));
                    }
                    ...
                    public static void syncTask(int questId, int taskId){//todo maybe add subtasks tasks in for loop instead of full task sync, otherwise datacap crash will happen again
                        Constants.currentServerInstance.getPlayerList().getPlayers().forEach(player -> syncTaskToPlayer(player, questId, taskId));
                    }
                    ...
                    public static void syncSubtask(int questId, int taskId, int subtaskId){
                        Constants.currentServerInstance.getPlayerList().getPlayers().forEach(player -> syncSubtaskToPlayer(player, questId, taskId, subtaskId));
                    }
                    ...
                    public static void syncReward(int questId, int rewardId){//todo maybe add subrewards tasks in for loop instead of full reward sync, otherwise datacap crash will happen again
                        Constants.currentServerInstance.getPlayerList().getPlayers().forEach(player -> syncRewardToPlayer(player, questId, rewardId));
                    }
                ...
                public static class Chapters{
                    public static void syncAllChapters(){
                        Constants.currentServerInstance.getPlayerList().getPlayers().forEach(Chapters::syncAllChaptersToPlayer);
                    }
            ...
            public static class Config{
                public static void syncConfigToAllPlayers(){
                    Constants.currentServerInstance.getPlayerList().getPlayers().forEach(Config::syncConfigToPlayer);
                }
