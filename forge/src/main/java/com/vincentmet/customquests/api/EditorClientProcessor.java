...
public class EditorClientProcessor {
    ...
    public static class Create{
        ...
        public static void createTask(int questId, int taskId, JsonObject json){
            if(QuestHelper.doesQuestExist(questId)){
                Task t = new Task(questId, taskId, Constants.INVALID_RESOURCELOCATION);//gets updated by next line
                t.processJson(json);
    ...
    public static class Delete{ //Deleting it, including all references
        ...
        public static void deleteSingleParty(int partyId){
            Clear.Parties.clearSingleParty(partyId);
            QuestingStorage.getSidedPlayersMap().values().stream().filter(player -> player.getParty() == partyId).forEach(player -> player.setParty(Constants.NO_PARTY));//Sets the players party to none if they are in the party to delete
        }
