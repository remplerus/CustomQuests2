...
import com.vincentmet.customquests.events.PartyEvent;
...
public class PartyHelper {
	...
	public static void forEachPlayerInPartyCurrentlyOnline(int partyId, Consumer<ServerPlayer> consumer){
		getAllUUIDsInParty(partyId).stream().map(Constants.currentServerInstance.getPlayerList()::getPlayer).filter(Objects::nonNull).forEach(consumer);
	}
