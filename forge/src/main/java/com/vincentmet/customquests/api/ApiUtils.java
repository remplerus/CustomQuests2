...
public class ApiUtils{
	...
	public static void backupProgress(){
		File oldFile = Constants.currentProgressDirectory.resolve(Constants.FILENAME_PARTIES+ Constants.FILE_EXT_JSON).toFile();
		Path newPath = Constants.progressBackupDirectory;
		File newFile = new File(Constants.FILENAME_PARTIES + "_" + getLocalFormattedTime() + Constants.FILE_EXT_JSON);
		try{
			writeTo(newPath, newFile.toString(), Files.readString(oldFile.toPath()));
		}catch(IOException e){
			e.printStackTrace();
