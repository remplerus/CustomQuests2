...
public class QuestButton implements IJsonObjectProvider, IJsonObjectProcessor, IEditorPage {
    ...
    public void setIcon(IQuestingTexture icon){
        if (icon != null && icon.isValid()){
            ...
            CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > button > icon': The given texture is either null or invalid, defaulting to '"+ForgeRegistries.BLOCKS.getKey(Blocks.GRASS_BLOCK)+"'!");
    ...
    public void setIcon(ResourceLocation iconRL){
        if(TagHelper.Items.doesTagExist(iconRL)){
            ...
            if(ForgeRegistries.ITEMS.containsKey(iconRL)){
                ...
                CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > button > icon': There is no valid item/tag with ResourceLocation '" + iconRL + "' found, defaulting to '"+ForgeRegistries.BLOCKS.getKey(Blocks.GRASS_BLOCK)+"'!");
                setIcon(DEFAULT_ICON);
            }
        }
    }
        ...
        if(json.has("icon")){
            ...
            if(jsonElement.isJsonPrimitive()){
                ...
                if(jsonPrimitive.isString()){
                    ...
                    CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > button > icon': Value is not a String, defaulting to '"+ForgeRegistries.BLOCKS.getKey(Blocks.GRASS_BLOCK)+"'!");
                    setIcon(DEFAULT_ICON);
                }
            }else{
                CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > button > icon': Value is not a JsonPrimitive, please use a String, defaulting to '"+ForgeRegistries.BLOCKS.getKey(Blocks.GRASS_BLOCK)+"'!");
                setIcon(DEFAULT_ICON);
            }
        }else{
            CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > button > icon': Not detected, defaulting to '"+ForgeRegistries.BLOCKS.getKey(Blocks.GRASS_BLOCK)+"'!");
            setIcon(DEFAULT_ICON);
        }
    ...
    public void addPageEntries(List<IEditorEntry> list) {
        list.add(new EditorEntryWrapper(Component.translatable(Constants.MODID + ".editor.keys.shape"), ResourceLocation.fromNamespaceAndPath(Constants.MODID, "resourcelocation"), () -> getShape().getId().toString(), newValueObject -> {
        ...
        list.add(new EditorEntryWrapper(Component.translatable(Constants.MODID + ".editor.keys.scale"), ResourceLocation.fromNamespaceAndPath(Constants.MODID, "double"), this::getScale, newValueObject -> {
