			...
			if(jsonElement.isJsonPrimitive()){
				...
				if(jsonPrimitive.isString()){
					...
					if(operator.equalsIgnoreCase("AND") || operator.equalsIgnoreCase("OR")){
						...
						CustomQuestsLogger.warn("'Quest > " + questId + " > tasks > logic': Value is not a valid operator, please use 'AND' or 'OR', defaulting to 'AND'!");
						setLogicType(LogicType.AND);
					}
				}else{
					CustomQuestsLogger.warn("'Quest > " + questId + " > tasks > logic': Value is not a String, defaulting to 'AND'!");
					setLogicType(LogicType.AND);
				}
			}else{
				CustomQuestsLogger.warn("'Quest > " + questId + " > tasks > logic': Value is not a JsonPrimitive, please use a String, defaulting to 'AND'!");
				setLogicType(LogicType.AND);
			}
		}else{
			CustomQuestsLogger.warn("'Quest > " + questId + " > tasks > logic': Not detected, defaulting to 'AND'!");
			setLogicType(LogicType.AND);
		}
							...
							if(jsonObjectValueElement.isJsonPrimitive()){
								...
								if(jsonObjectValuePrimitive.isString()){
									...
									if(CQRegistry.getTaskTypes().keySet().stream().anyMatch(tasktypeId -> tasktypeId.toString().equals(jsonStringType))){
										Task task = new Task(questId, keyInt, ResourceLocation.tryParse(jsonStringType));
										//task.processJson(jsonObjectValue);
										put(Integer.parseInt(key), task);
									}else{
										CustomQuestsLogger.error("'Quest > " + questId + " > tasks > entries > " + counter.getValue() + " > type': Value does not match a registered TaskType, please download the addon mod it belongs to, or change it to something valid, discarding it for now! THIS ERROR SHOULDN'T HAPPEN!");
									}
								}else{
									CustomQuestsLogger.error("'Quest > " + questId + " > tasks > entries > " + counter.getValue() + " > type': Value is not a String, discarding it for now! THIS ERROR SHOULDN'T HAPPEN!");
								}
							}else{
								CustomQuestsLogger.error("'Quest > " + questId + " > tasks > entries > " + counter.getValue() + " > type': Value is not a JsonPrimitive, please use a String, discarding it for now! THIS ERROR SHOULDN'T HAPPEN!");
							}
						}else{
							CustomQuestsLogger.error("'Quest > " + questId + " > tasks > entries > " + counter.getValue() + " > type': Not detected, discarding it for now! THIS ERROR SHOULDN'T HAPPEN!");
						}
					}else{
