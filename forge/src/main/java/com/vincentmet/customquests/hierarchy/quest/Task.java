	...
	public void processJson(JsonObject json){
			...
			if(jsonElement.isJsonPrimitive()){
				...
				if(jsonPrimitive.isString()){
					...
					if(CQRegistry.getTaskTypes().keySet().stream().anyMatch(tasktypeId -> tasktypeId.toString().equals(jsonString))){
						setTaskType(ResourceLocation.tryParse(jsonString));
					}else{
						CustomQuestsLogger.error("'Quest > " + questId + " > tasks > entries > " + taskId + " > type': Value does not match a registered TaskType, please download the addon mod it belongs to, or change it to something valid, discarding it for now! THIS ERROR SHOULDN'T HAPPEN!");
					}
				}else{
					CustomQuestsLogger.error("'Quest > " + questId + " > tasks > entries > " + taskId + " > type': Value is not a String, discarding it for now! THIS ERROR SHOULDN'T HAPPEN!");
				}
			}else{
				CustomQuestsLogger.error("'Quest > " + questId + " > tasks > entries > " + taskId + " > type': Value is not a JsonPrimitive, please use a String, discarding it for now! THIS ERROR SHOULDN'T HAPPEN!");
			}
		}else{
			CustomQuestsLogger.error("'Quest > " + questId + " > tasks > entries > " + taskId + " > type': Not detected, discarding it for now! THIS ERROR SHOULDN'T HAPPEN!");
		}
		...
		if(json.has("sub_tasks")){
			...
			if(jsonElement.isJsonObject()){
				...
				CustomQuestsLogger.warn("'Quest > " + questId + " > tasks > entries > " + taskId + " > sub_tasks': Value is not a JsonObject, generating a new one!");
				subtasks.processJson(new JsonObject());
			}
		}else{
			CustomQuestsLogger.warn("'Quest > " + questId + " > tasks > entries > " + taskId + " > sub_tasks': Not detected, generating a new JsonObject!");
			subtasks.processJson(new JsonObject());
		}
	}
