		...
		if(json.has("all_subtasks_completed")){
			...
			if(jsonElement.isJsonPrimitive()){
				...
				if(jsonPrimitive.isBoolean()){
					...
					CustomQuestsLogger.warn("'User > " + uuid.toString() + " > individual_progress > entries > " + questId + " > task_progress > " + taskId + " > all_subtasks_completed': Value is not a Boolean, defaulting to 'false'!");
					setAllSubtasksCompleted(false);
				}
			}else{
				CustomQuestsLogger.warn("'User > " + uuid.toString() + " > individual_progress > entries > " + questId + " > task_progress > " + taskId + " > all_subtasks_completed': Value is not a JsonPrimitive, please use a boolean, defaulting to 'false'!");
				setAllSubtasksCompleted(false);
			}
		}else{
			CustomQuestsLogger.warn("'User > " + uuid.toString() + " > individual_progress > entries > " + questId + " > task_progress > " + taskId + " > all_subtasks_completed': Not detected, defaulting to 'false'!");
			setAllSubtasksCompleted(false);
		}
			...
			if(jsonElement.isJsonObject()){
				...
				for(Map.Entry<String, JsonElement> jsonEntryElement : jsonObject.entrySet()){
					...
					if(value.isJsonObject()){
						...
						CustomQuestsLogger.warn("'User > " + uuid.toString() + " > individual_progress > entries > " + questId + " > task_progress > " + taskId + " > subtask_progress > " + counter.getValue() + "': Value is not a JsonObject, discarding it for now!");
					}
					counter.count();
				}
			}else{
				CustomQuestsLogger.warn("'User > " + uuid.toString() + " > individual_progress > entries > " + questId + " > task_progress > " + taskId + " > subtask_progress': Value is not a JsonObject, generating a new one!");
			}
		}else{
			CustomQuestsLogger.warn("'User > " + uuid.toString() + " > individual_progress > entries > " + questId + " > task_progress > " + taskId + " > subtask_progress': Not detected, generating a new JsonObject!");
		}
	}
