	...
	public void processJson(JsonObject json){
		if(json.has("party")){
			...
			if(jsonElement.isJsonPrimitive()){
				...
				if(jsonPrimitive.isNumber()){
					...
					if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'User > " + uuid.toString() + " > party': Value is not an Integer, defaulting to '-1'!");
					setParty(-1);
				}
			}else{
				if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'User > " + uuid.toString() + " > party': Value is not a JsonPrimitive, please use an Integer, defaulting to '-1'!");
				setParty(-1);
			}
		}else{
		...
		if(json.has("individual_progress")){
			...
			if(jsonElement.isJsonObject()){
				...
				if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'User > " + uuid.toString() + " > individual_progress': Value is not a JsonObject, generating a new one!");
				individualProgress.processJson(new JsonObject());
			}
		}else{
			if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'User > " + uuid.toString() + " > individual_progress': Not detected, generating a new JsonObject!");
			individualProgress.processJson(new JsonObject());
		}
	}
