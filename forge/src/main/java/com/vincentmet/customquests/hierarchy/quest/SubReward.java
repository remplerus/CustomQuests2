	...
	public void processJson(JsonObject json){
		...
		if(json.has("content")){
			...
			if(jsonElement.isJsonObject()){
				...
				CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > rewards > entries > " + parentRewardId + " > entries > " + subRewardId + " > content': Value is not a JsonObject, generating a new one!");
				CQRegistry.getRewardTypes().entrySet()
						  .stream()
						  .filter(entry -> entry.getKey().toString().equals(rewardType.toString()))
						  .forEach(entry->{
			...
			CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > rewards > entries > " + parentRewardId + " > entries > " + subRewardId + " > content': Not detected, generating a new JsonObject!");
			CQRegistry.getRewardTypes().entrySet()
					  .stream()
					  .filter(entry -> entry.getKey().toString().equals(rewardType.toString()))
					  .forEach(entry->{
