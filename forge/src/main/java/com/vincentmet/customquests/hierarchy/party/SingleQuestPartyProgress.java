		...
		if(json.has("claimed")){
			...
			if(jsonElement.isJsonPrimitive()){
				...
				if(jsonPrimitive.isBoolean()){
					...
					if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'Party > " + partyId + " > collective_progress > " + questId + " > claimed': Value is not a Boolean, defaulting to 'false'!");
					setClaimed(false);
				}
			}else{
				if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'Party > " + partyId + " > collective_progress > " + questId + " > claimed': Value is not a JsonPrimitive, please use a boolean, defaulting to 'false'!");
				setClaimed(false);
			}
		}else{
			if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'Party > " + partyId + " > collective_progress > " + questId + " > claimed': Not detected, defaulting to 'false'!");
			setClaimed(false);
		}
		...
		if(json.has("all_tasks_completed")){
			...
			if(jsonElement.isJsonPrimitive()){
				...
				if(jsonPrimitive.isBoolean()){
					...
					if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'Party > " + partyId + " > collective_progress > " + questId + " > all_tasks_completed': Value is not a Boolean, defaulting to 'false'!");
					setAllTasksCompleted(false);
				}
			}else{
				if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'Party > " + partyId + " > collective_progress > " + questId + " > all_tasks_completed': Value is not a JsonPrimitive, please use a boolean, defaulting to 'false'!");
				setAllTasksCompleted(false);
			}
		}else{
			if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'Party > " + partyId + " > collective_progress > " + questId + " > all_tasks_completed': Not detected, defaulting to 'false'!");
			setAllTasksCompleted(false);
		}
			...
			if(jsonElement.isJsonObject()){
				...
				for(Entry<String, JsonElement> jsonEntryElement : jsonObject.entrySet()){
					...
					if(value.isJsonObject()){
						...
						if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'Party > " + partyId + " > collective_progress > " + questId + " > task_progress > " + counter.getValue() + "': Value is not a JsonObject, discarding it for now!");
					}
					counter.count();
				}
			}else{
				if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'Party > " + partyId + " > collective_progress > " + questId + " > task_progress': Value is not a JsonObject, generating a new one!");
			}
		}else{
			if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'Party > " + partyId + " > collective_progress > " + questId + " > task_progress': Not detected, generating a new JsonObject!");
		}
	}
