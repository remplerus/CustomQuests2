		...
		if(json.has("all_subtasks_completed")){
			...
			if(jsonElement.isJsonPrimitive()){
				...
				if(jsonPrimitive.isBoolean()){
					...
					if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'Party > " + partyId + " > collective_progress > " + questId + " > task_progress > " + taskId + " > all_subtasks_completed': Value is not a Boolean, defaulting to 'false'!");
					setAllSubtasksCompleted(false);
				}
			}else{
				if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'Party > " + partyId + " > collective_progress > " + questId + " > task_progress > " + taskId + " > all_subtasks_completed': Value is not a JsonPrimitive, please use a boolean, defaulting to 'false'!");
				setAllSubtasksCompleted(false);
			}
		}else{
			if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'Party > " + partyId + " > collective_progress > " + questId + " > task_progress > " + taskId + " > all_subtasks_completed': Not detected, defaulting to 'false'!");
			setAllSubtasksCompleted(false);
		}
			...
			if(jsonElement.isJsonObject()){
				...
				for(Entry<String, JsonElement> jsonEntryElement : jsonObject.entrySet()){
					...
					if(value.isJsonObject()){
						...
						if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'Party > " + partyId + " > collective_progress > " + questId + " > task_progress > " + taskId + " > subtask_progress > " + counter.getValue() + "': Value is not a JsonObject, discarding it for now!");
					}
					counter.count();
				}
			}else{
				if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'Party > " + partyId + " > collective_progress > " + questId + " > task_progress > " + taskId + " > subtask_progress': Value is not a JsonObject, generating a new one!");
			}
		}else{
			if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'Party > " + partyId + " > collective_progress > " + questId + " > task_progress > " + taskId + " > subtask_progress': Not detected, generating a new JsonObject!");
		}
	}
