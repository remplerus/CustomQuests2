			...
			if(jsonElement.isJsonPrimitive()){
				...
				if(jsonPrimitive.isString()){
					...
					if(operator.equalsIgnoreCase("AND") || operator.equalsIgnoreCase("OR")){
						...
						CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > dependencies > logic': Value is not a valid operator, please use 'AND' or 'OR', defaulting to 'AND'!");
						setLogicType(LogicType.AND);
					}
				}else{
					CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > dependencies > logic': Value is not a String, defaulting to 'AND'!");
					setLogicType(LogicType.AND);
				}
			}else{
				CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > dependencies > logic': Value is not a JsonPrimitive, please use a String, defaulting to 'AND'!");
				setLogicType(LogicType.AND);
			}
		}else{
			CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > dependencies > logic': Not detected, defaulting to 'AND'!");
			setLogicType(LogicType.AND);
		}
				...
				for(JsonElement jsonEntriesElement : jsonArray){
					if(jsonEntriesElement.isJsonPrimitive()){
						...
						if(jsonEntriesPrimitive.isNumber()){
							...
							CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > dependencies > entries > " + counter.getValue() + "': Value is not an Integer, discarding it for now!");
						}
					}else{
						CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > dependencies > entries > " + counter.getValue() + "': Value is not a JsonPrimitive, please use an Integer, discarding it for now!");
					}
					counter.count();
				}
			}else{
				CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > dependencies > entries': Value is not a JsonArray, generating a new one!");
			}
		}else{
			CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > dependencies > entries': Not detected, generating a new JsonArray!");
		}
	}
