		...
		if(json.has("claimed")){
			...
			if(jsonElement.isJsonPrimitive()){
				...
				if(jsonPrimitive.isBoolean()){
					...
					CustomQuestsLogger.warn("'User > " + uuid.toString() + " > individual_progress > entries > " + questId + " > claimed': Value is not a Boolean, defaulting to 'false'!");
					setClaimed(false);
				}
			}else{
				CustomQuestsLogger.warn("'User > " + uuid.toString() + " > individual_progress > entries > " + questId + " > claimed': Value is not a JsonPrimitive, please use a boolean, defaulting to 'false'!");
				setClaimed(false);
			}
		}else{
			CustomQuestsLogger.warn("'User > " + uuid.toString() + " > individual_progress > entries > " + questId + " > claimed': Not detected, defaulting to 'false'!");
			setClaimed(false);
		}
		...
		if(json.has("all_tasks_completed")){
			...
			if(jsonElement.isJsonPrimitive()){
				...
				if(jsonPrimitive.isBoolean()){
					...
					CustomQuestsLogger.warn("'User > " + uuid.toString() + " > individual_progress > entries > " + questId + " > all_tasks_completed': Value is not a Boolean, defaulting to 'false'!");
					setAllTasksCompleted(false);
				}
			}else{
				CustomQuestsLogger.warn("'User > " + uuid.toString() + " > individual_progress > entries > " + questId + " > all_tasks_completed': Value is not a JsonPrimitive, please use a boolean, defaulting to 'false'!");
				setAllTasksCompleted(false);
			}
		}else{
			CustomQuestsLogger.warn("'User > " + uuid.toString() + " > individual_progress > entries > " + questId + " > all_tasks_completed': Not detected, defaulting to 'false'!");
			setAllTasksCompleted(false);
		}
			...
			if(jsonElement.isJsonObject()){
				...
				for(Map.Entry<String, JsonElement> jsonEntryElement : jsonObject.entrySet()){
					...
					if(value.isJsonObject()){
						...
						CustomQuestsLogger.warn("'User > " + uuid.toString() + " > individual_progress > entries > " + questId + " > task_progress > " + counter.getValue() + "': Value is not a JsonObject, discarding it for now!");
					}
					counter.count();
				}
			}else{
				CustomQuestsLogger.warn("'User > " + uuid.toString() + " > individual_progress > entries > " + questId + " > task_progress': Value is not a JsonObject, generating a new one!");
			}
		}else{
			CustomQuestsLogger.warn("'User > " + uuid.toString() + " > individual_progress > entries > " + questId + " > task_progress': Not detected, generating a new JsonObject!");
		}
	}
