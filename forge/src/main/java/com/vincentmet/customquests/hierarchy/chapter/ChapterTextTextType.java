...
public class ChapterTextTextType implements IJsonObjectProcessor, IJsonObjectProvider, IEditorPage {
        ...
        if(json.has("type")){
            ...
            if(jsonElement.isJsonPrimitive()){
                ...
                if(jsonPrimitive.isString()){
                    ...
                    CustomQuestsLogger.warn("'Chapter > " + parentId + " > text > type': Value is not a String, defaulting to 'customquests:plaintext'!");
                    setType(new PlainTextTextType());
                }
            }else{
                CustomQuestsLogger.warn("'Chapter > " + parentId + " > text > type': Value is not a JsonPrimitive, please use a String, defaulting to 'customquests:plaintext'!");
                setType(new PlainTextTextType());
            }
        }else{
            CustomQuestsLogger.warn("'Chapter > " + parentId + " > text > type': Not detected, defaulting to 'customquests:plaintext'!");
            setType(new PlainTextTextType());
        }
        ...
        if(json.has("text")){
            ...
            if(jsonElement.isJsonPrimitive()){
                ...
                if(jsonPrimitive.isString()){
                    ...
                    CustomQuestsLogger.warn("'Chapter > " + parentId + " > text > text': Value is not a String, defaulting to an empty String!");
                }
            }else{
                CustomQuestsLogger.warn("'Chapter > " + parentId + " > text > text': Value is not a JsonPrimitive, please use a String, defaulting to an empty String!");
            }
        }else{
            CustomQuestsLogger.warn("'Chapter > " + parentId + " > text > text': Not detected, defaulting to an empty String!");
        }
    }
    ...
    public void setType(ITextType type){
        if(type != null){
            ...
            CustomQuestsLogger.warn("'Chapter > " + parentId + " > text > type': Value does not match a registered TextType or is null, please download the addon mod it belongs to, or change it to something valid. Defaulting to 'customquests:plaintext'");
            setType(new PlainTextTextType());
        }
    }
