	...
	public void processJson(JsonObject json){
		if(json.has("quests_individually_completed")){
			...
			if(jsonElement.isJsonArray()){
				...
				if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'User > " + uuid.toString() + " > individual_progress > quests_individually_completed': Value is not a JsonArray, generating a new one!");
				individuallyCompletedQuests.processJson(new JsonArray());
			}
		}else{
			if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'User > " + uuid.toString() + " > individual_progress > quests_individually_completed': Not detected, generating a new JsonArray!");
			individuallyCompletedQuests.processJson(new JsonArray());
		}
			...
			if(jsonElement.isJsonObject()){
				...
				for(Map.Entry<String, JsonElement> jsonEntryElement : jsonObject.entrySet()){
					...
					if(value.isJsonObject()){
						...
						if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'User > " + uuid.toString() + " > individual_progress > entries > " + counter.getValue() + "': Value is not a JsonObject, discarding it for now!");
					}
					counter.count();
				}
			}else{
				if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'User > " + uuid.toString() + " > individual_progress > entries': Value is not a JsonObject, generating a new one!");
			}
		}else{
			if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'User > " + uuid.toString() + " > individual_progress > entries': Not detected, generating a new JsonObject!");
		}
	}
