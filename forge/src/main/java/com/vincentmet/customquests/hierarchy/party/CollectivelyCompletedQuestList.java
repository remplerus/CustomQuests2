		...
		for(JsonElement jsonEntriesElement : json){
			if(jsonEntriesElement.isJsonPrimitive()){
				...
				if(jsonEntriesPrimitive.isNumber()){
					...
					if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'Party > " + parentPartyId + " > quests_collectively_completed > " + counter.getValue() + "': Value is not an Integer, discarding it for now!");
				}
			}else{
				if(Config.SidedConfig.isDebugModeOn()) CustomQuestsLogger.warn("'User > " + parentPartyId + " > quests_collectively_completed > " + counter.getValue() + "': Value is not a JsonPrimitive, please use an Integer, discarding it for now!");
			}
			counter.count();
		}
	}
