			...
			if(jsonElement.isJsonPrimitive()){
				...
				if(jsonPrimitive.isString()){
					...
					if(operator.equalsIgnoreCase("AND") || operator.equalsIgnoreCase("OR")){
						...
						CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > rewards > logic': Value is not a valid operator, please use 'AND' or 'OR', defaulting to 'AND'!");
						setLogicType(LogicType.AND);
					}
				}else{
					CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > rewards > logic': Value is not a String, defaulting to 'AND'!");
					setLogicType(LogicType.AND);
				}
			}else{
				CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > rewards > logic': Value is not a JsonPrimitive, please use a String, defaulting to 'AND'!");
				setLogicType(LogicType.AND);
			}
		}else{
			CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > rewards > logic': Not detected, defaulting to 'AND'!");
			setLogicType(LogicType.AND);
		}
			...
			if(jsonElement.isJsonObject()){
				...
				for(Map.Entry<String, JsonElement> jsonEntryElement : jsonObject.entrySet()){
					...
					if(value.isJsonObject()){
						...
						CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > tasks > entries > " + counter.getValue() + "': Value is not a JsonObject, discarding it for now!");
					}
					counter.count();
				}
			}else{
				CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > rewards > entries': Value is not a JsonObject, generating a new one!");
			}
		}else{
			CustomQuestsLogger.warn("'Quest > " + parentQuestId + " > rewards > entries': Not detected, generating a new JsonObject!");
		}
	}
