...
public class PredicateTexture implements IQuestingTexture{
    ...
    @OnlyIn(Dist.CLIENT)
	public void render(GuiGraphics guiGraphics, float scale, int x, int y, float offsetX, float offsetY, int mouseX, int mouseY){
		if(showing.get()){
			PoseStack matrixStack = guiGraphics.pose();
			matrixStack.pushPose();
			matrixStack.scale(scale, scale, 1);
			RenderSystem.setShaderTexture(0, texture);
			guiGraphics.blit(texture, x, y, texU, texV, texWidth, texHeight, texSizeX, texSizeY);
			matrixStack.popPose();
		}
	}
