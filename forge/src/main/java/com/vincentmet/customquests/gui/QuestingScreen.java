...
public class QuestingScreen extends Screen {
	...
	private static final Component localization_noQuests = Component.translatable(Constants.MODID + ".screens.no_quests");
	private static final Component localization_backToChapters = Component.translatable(Constants.MODID + ".screens.back_to_chapters");
	
	private final List<Component> tooltip_backToChapters = new ArrayList<>();
	
	public QuestingScreen(OptionalInt questId){
		super(Component.translatable("item." + Constants.MODID + ".questing_device"));
	...
	private void renderBackgrounds(GuiGraphics matrixStack){
		...
		if(!screenManager.shouldShowQuestDetails()){
			matrixStack.fill(chapterListX.getAsInt(), chapterListY.getAsInt(), chapterListX.getAsInt() + chapterListWidth.getAsInt(), chapterListY.getAsInt() + chapterListHeight.getAsInt(), 0x88000000);
			matrixStack.fill(questingCanvasX.getAsInt(), questingCanvasY.getAsInt(), questingCanvasX.getAsInt() + questingCanvasWidth.getAsInt(), questingCanvasY.getAsInt() + questingCanvasHeight.getAsInt(), 0x88000000);
		}
	}
