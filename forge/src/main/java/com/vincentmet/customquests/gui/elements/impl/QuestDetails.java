...
public class QuestDetails implements IHoverRenderable, CQGuiEventListener {
	...
	private final VariableButton claimRewardButton = new VariableButton(()->0, ()->0, CLAIM_BUTTON_WIDTH, CLAIM_BUTTON_HEIGHT, ButtonState.DISABLED.getButtonTexture(), Component.translatable(Constants.MODID + ".screens.claim").getString(), new Vec2i(0, 0), (mouseButton)->{PacketHandler.CHANNEL.sendToServer(new MessageRewardClaim(screenManager.getCurrentlySelectedQuestId(), selectedRewardId.get()));}, new ArrayList<>());
	...
	public void reInitTasks(){
		if(screenManager.getCurrentlySelectedQuestId()>=0){
			String taskLogic = ChatFormatting.GRAY + Component.translatable(Constants.MODID + ".general.type").getString() + ": [" + QuestingStorage.getSidedQuestsMap().get(screenManager.getCurrentlySelectedQuestId()).getTasks().getLogicType() + "]";
	...
	public void reInitRewards(){
		if(QuestHelper.doesQuestExist(screenManager.getCurrentlySelectedQuestId())){
			...
			if(((QuestHelper.doesRewardExist(screenManager.getCurrentlySelectedQuestId(), selectedRewardId.get()) && QuestingStorage.getSidedQuestsMap().get(screenManager.getCurrentlySelectedQuestId()).getRewards().getLogicType().equals(LogicType.OR)) || QuestingStorage.getSidedQuestsMap().get(screenManager.getCurrentlySelectedQuestId()).getRewards().getLogicType().equals(LogicType.AND)) && !CombinedProgressHelper.isQuestClaimed(clientPlayer.getUUID(), screenManager.getCurrentlySelectedQuestId()) && CombinedProgressHelper.isQuestCompleted(clientPlayer.getUUID(), screenManager.getCurrentlySelectedQuestId())){
				...
				claimRewardButton.setButtonText(Component.translatable(Constants.MODID + ".screens.claim").getString());
				...
				claimRewardButton.setButtonText(Component.translatable(Constants.MODID + ".screens.claimed").getString());
			}else{
				claimRewardButton.setTexture(ButtonState.DISABLED.getButtonTexture());
				claimRewardButton.setButtonText(Component.translatable(Constants.MODID + ".screens.claim").getString());
			}
	...
	private void renderHoverBackground(GuiGraphics matrixStack, int mouseX, int mouseY, float partialTicks){/*NOOP*/}
	private void renderHoverText(GuiGraphics matrixStack, int mouseX, int mouseY, float partialTicks){/*NOOP*/}
	
	private void renderTasks(GuiGraphics matrixStack, int mouseX, int mouseY, float partialTicks){
	...
	private void renderHoverTasks(GuiGraphics matrixStack, int mouseX, int mouseY, float partialTicks){
		GLScissorStack.push(matrixStack, taskContentX.getAsInt(), taskContentY.getAsInt(), taskContentWidth.getAsInt(), taskContentHeight.getAsInt());
		taskButtons.stream()
	...
	private void renderRewards(GuiGraphics matrixStack, int mouseX, int mouseY, float partialTicks){
	...
	private void renderHoverRewards(GuiGraphics matrixStack, int mouseX, int mouseY, float partialTicks){
		GLScissorStack.push(matrixStack, rewardContentX.getAsInt(), rewardContentY.getAsInt(), rewardContentWidth.getAsInt(), rewardContentHeight.getAsInt());
	...
	private void renderBackground(GuiGraphics matrixStack, int mouseX, int mouseY, float partialTicks){
		GLScissorStack.push(matrixStack, textX.getAsInt(), textY.getAsInt(), textWidth.getAsInt(), textHeight.getAsInt());
		matrixStack.fill(textX.getAsInt(), textY.getAsInt(), textX.getAsInt() + textWidth.getAsInt(), textY.getAsInt() + textHeight.getAsInt(), 0x88000000);
		GLScissorStack.pop(matrixStack);
		
		GLScissorStack.push(matrixStack, tasksX.getAsInt(), tasksY.getAsInt(), tasksWidth.getAsInt(), tasksHeight.getAsInt());
		matrixStack.fill(tasksX.getAsInt(), tasksY.getAsInt(), tasksX.getAsInt() + tasksWidth.getAsInt(), tasksY.getAsInt() + tasksHeight.getAsInt(), 0x88000000);
		GLScissorStack.pop(matrixStack);
		
		GLScissorStack.push(matrixStack, rewardsX.getAsInt(), rewardsY.getAsInt(), rewardsWidth.getAsInt(), rewardsHeight.getAsInt());
		matrixStack.fill(rewardsX.getAsInt(), rewardsY.getAsInt(), rewardsX.getAsInt() + rewardsWidth.getAsInt(), rewardsY.getAsInt() + rewardsHeight.getAsInt(), 0x88000000);
		GLScissorStack.pop(matrixStack);
	}
