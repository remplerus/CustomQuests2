...
public class SingleLineTextField implements IHoverRenderable, CQGuiEventListener {
    ...
    private void renderCursor(GuiGraphics matrixStack, int mouseX, int mouseY, float partialTicks){
        if(isFocused && System.currentTimeMillis()%1000>=500){
            matrixStack.fill(TEXTAREA_X.getAsInt() + getCursorPosInPx() - getOffset(), TEXTAREA_Y.getAsInt()-1, TEXTAREA_X.getAsInt() + getCursorPosInPx() - getOffset() + 1, TEXTAREA_Y.getAsInt() + TEXTAREA_HEIGHT.getAsInt() + 1, cursorColor);
        }
    }
